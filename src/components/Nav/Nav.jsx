  ...
  return (
    <header className="fixed top-0 left-0 right-0 w-full bg-white z-40 flex flex-row justify-between items-center px-6 lg:px-16 py-4 shadow-md">
      ...
      >
        ...
        <span className={`block w-6 h-0.5 bg-black mt-1.5 transition-opacity duration-300 ease-in-out ${isMenuOpen ? 'opacity-0' : ''}`}></span>
        <span className={`block w-6 h-0.5 bg-black mt-1.5 transition-transform duration-300 ease-in-out ${isMenuOpen ? '-rotate-45 -translate-y-1' : ''}`}></span>
      </button>
